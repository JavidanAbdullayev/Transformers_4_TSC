...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
   ...
   # val_times["total_time"] += eval_runtime
    ...
    print('\n\n\n Test aggr_metrics: ', aggr_metrics, '\n\n\n')
    print()
...
def train_runner(config, model, trainer, val_evaluator, path):
    ...
    for epoch in tqdm(range(start_epoch + 1, epochs + 1), desc='Training Epoch', leave=False):
        ...
        # Save best model
        if min_train_loss  > aggr_metrics_train['loss']:
            min_train_loss = aggr_metrics_train['loss']
            best_model_wts = copy.deepcopy(model.state_dict())
            save_best_model(aggr_metrics_train['loss'], epoch, model, optimizer, loss_module, path)


        print('Training loss: ', aggr_metrics_train['loss'])

        with torch.no_grad():
            aggr_metrics_val, ConfMat = val_evaluator.evaluate(epoch, keep_all=True)
        print('Alone Test perfromance: ', aggr_metrics_val)
        ...
        break    
    torch.save(best_model_wts, os.path.join(config['save_dir'], 'model_best.pth'))
